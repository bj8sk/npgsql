...
ORDER BY oid{(withEnumSortOrder ? ", enumsortorder" : "")};";
    ...
    internal async Task<List<PostgresType>> LoadBackendTypes(NpgsqlConnector conn, NpgsqlTimeout timeout, bool async)
    {
        ...
        var loadTypesQuery = GenerateLoadTypesQuery(SupportsRangeTypes, SupportsMultirangeTypes, conn.Settings.LoadTableComposites, conn.Settings.LoadTypesFromSearchPath, schemasFormatted, HasTypeCategory);
        var loadCompositeTypesQuery = GenerateLoadCompositeTypesQuery(conn.Settings.LoadTableComposites, conn.Settings.LoadTypesFromSearchPath, schemasFormatted, HasTypeCategory);
