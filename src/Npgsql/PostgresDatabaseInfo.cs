...
ORDER BY oid{(withEnumSortOrder ? ", enumsortorder" : "")};";
    ...
    internal async Task<List<PostgresType>> LoadBackendTypes(NpgsqlConnector conn, NpgsqlTimeout timeout, bool async)
    {
        ...
        var loadTypesQuery = GenerateLoadTypesQuery(SupportsRangeTypes, SupportsMultirangeTypes, conn.Settings.LoadTableComposites, conn.Settings.LoadOnlyCompositeFromSearchPath, schemasFormatted);
        var loadCompositeTypesQuery = GenerateLoadCompositeTypesQuery(conn.Settings.LoadTableComposites, conn.Settings.LoadOnlyCompositeFromSearchPath, schemasFormatted);
